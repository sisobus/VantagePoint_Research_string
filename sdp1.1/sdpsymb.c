              ...
              char *info)
{
  ...
  if (n) {
    for (i=0; i<m; i++) {
      ...
      if (!ipt[i]) ExitProc(OutOfSpc,info);
    }
  }
} /* AllocIptr */
                             ...
                             int   *i6nrow)
{
  ...
  sf->dhead=iAlloc(ndens+1,  "sf->dhead, DetectDenseNodes");
  sf->dsub=iAlloc(sf->nsnds,"sf->dsub, DetectDenseNodes");
  sf->dbeg=iAlloc(sf->nsnds,"sf->dbeg, DetectDenseNodes");
                   ...
                   int   ulnnz)
{
  ...
  pssub=iAlloc(sf->snnz,"pssub, ChlSymb");
  ...
  tusub=iAlloc(ffree,"tusub, ChlSymb");
  ...
  if (ffree<=sf->ujnz) {
    ...
    sf->usub=iAlloc(ffree,"sf->usub, ChlSymb");
    iCopy(ffree,usub,sf->usub);
  ...
  i1nrow=iAlloc(4*nrow,"i1nrow, ChlSymb");
  i2nrow=i1nrow+nrow;
  i3nrow=i2nrow+nrow;
  i4nrow=i3nrow+nrow;
...
int PspSymbo(symat *cy,
             chfac *cf)
{
  int   i,k,t,snnz,lnnz,*nnzi,
        nrow=cf->nrow,resp;
  order *od;
  ...
  cf->ssub=iAlloc(snnz,"cf, PspSymbo");
  cf->snnz=snnz;
  ...
  od  =OdAlloc(nrow,2*cf->snnz,"od, PspSymbo");
  nnzi=cf->perm;
...
int InvSymbo(syoff *cy,
             chfac *cf)
{
  int     i,k,snnz,lnnz,*nnzi,
          nrow=cf->nrow,resp;

               ...
               char *info)
{
  ...
  r->adjn=iAlloc(nn0,info);
  r->rbeg=iAlloc(nnod,info);
  r->rexs=iAlloc(nnod,info);
  r->rlen=iAlloc(nnod,info);
  r->rend=iAlloc(nnod,info);
  r->pres=iAlloc(nnod,info);
  r->succ=iAlloc(nnod,info);
             ...
             int   *p)
{
  ...
  xt=XtAlloc(n,n+1,"xt, GetOrder");
  iwmd=iAlloc(n,"ibuf21, GetOrder");
